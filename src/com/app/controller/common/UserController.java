...
public class UserController {
	...
	public ModelAndView processSignInForm(Model map, HttpSession hs, @RequestBody User incommingUser) {
		...
		try {
			...
			map.addAttribute("errorMsg_loginFailed_requestScopeSession", "Invalid credential, Please try again....");
			e.printStackTrace();
	...
	// ==========================signup : post=========================
	@PostMapping("/signup")
	public ModelAndView processSignUpForm(HttpSession hs, Model map, @RequestBody User incommingUser) {
		...
		Integer userId = iUserDao.signUpUserWithDetails(userSignUpDetails);
		
		System.out.println("=======userId========"+ userId);
		
		hs.setAttribute("signUpUserId", userId);
